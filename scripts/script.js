...
function sortFunc(a, b){
    ...
    if(a.price.newUan == undefined)
      aprice = parseInt(a.price.replace(/ /g,''));
    else
      aprice = parseInt(a.price.newUan.replace(/ /g,''));
    
    if(b.price.newUan == undefined)
      bprice = parseInt(b.price.replace(/ /g,''));
    else
      bprice = parseInt(b.price.newUan.replace(/ /g,''));
...
function createPage(){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        phoneTitle.innerHTML = data[i].name;

        

        container.appendChild(phone);
        phone.appendChild(dataBox);
        phone.appendChild(imgBox);
        imgBox.appendChild(imgPic);
        dataBox.appendChild(phoneTitle);
        
        if(data[i].price.newUan == undefined){
            let price = document.createElement("p");
            price.className = "singlePrice";
            price.innerHTML = data[i].price;
            dataBox.appendChild(price);
        }
        else{
              let oldPrice = document.createElement("p");
              oldPrice.className = "oldPrice";
              oldPrice.innerHTML = data[i].price.oldUan;
              dataBox.appendChild(oldPrice);
              
              let newPrice = document.createElement("p");
              newPrice.className = "newPrice";
              newPrice.innerHTML = data[i].price.newUan;
              dataBox.appendChild(newPrice);
        }

        let phoneRating = document.createElement("p");
        phoneRating.className = "phoneRating";
        phoneRating.innerHTML = data[i].ratingRevievs;
        dataBox.appendChild(phoneRating);
        
    }

    let footer = document.createElement("div");
    footer.className = "footer";
    footer.innerHTML = "Roman Mykhaylenko &copy; 2020 All Rights Reserved";
